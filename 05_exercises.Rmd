...
garden_harvest_graph <- garden_harvest %>%
  ggplot(aes(x = vegetable)) +
  geom_bar(fill = "dark green") + 
  coord_flip() +
  labs(title = "Count of Vegetables Grown in the Garden", 
       x = "",
       y = "") 
...
```{r}
covid_graph <- covid19 %>% 
  filter(state == "Minnesota"|state =="Wisconsin"|state =="Iowa"|state =="North   Dakota"|state =="South Dakota") %>% 
...
tomatoes <- garden_harvest %>% 
  ...
  mutate(total_tom_harv = cumsum(daily_harvest_lb)) %>% 
  ungroup() %>% 
  complete(variety, date, fill = list(daily_harvest_lb = 0)) 

tomato_animate <- tomatoes %>% 
  ggplot(aes(x = date, 
             y = fct_reorder(variety,total_tom_harv))) +
...
covid2 <- covid19 %>%
  group_by(state) %>% 
  mutate(lag_week = lag(cases, n = 7, order_by = date)) %>%
  replace_na(list(lag_day = 0, lag_week=0)) %>%
  mutate(newWeeklyCases = (cases - lag_week)) %>%
  filter(cases >= 20)
```

```{r}
covidAnimated <- covid2 %>%
  ggplot(aes(x = cases, y = newWeeklyCases, group = state)) +
...
animate(covidAnimated, nframes = 200, duration = 30)  
```

New York shot ahead of all the other states and then slowly was overcome by California and Florida. Most of the states have very similar trajectories other than the Virgin Islands and Guam.
...
covidMap <- covid3 %>% 
  ...
  labs(title = "US COVID-19 Cases per 10,000 People", 
       ...
       caption = "Grace McKenney",
       fill = "Cases per 10,000") +
  scale_fill_viridis_c() +
  transition_states(date, transition_length = 0) 

animate(covidMap, duration = 30)
  
```

The gif clearly shows that Washington is where the pandemic started and then it slowly spread through out the country. Then the patterns of covid are clearly shown throughout the map.
